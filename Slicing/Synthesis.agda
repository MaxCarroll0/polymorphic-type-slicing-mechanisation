...
private
  ...
  -- Reduce ⊥ₛ ⊔ₛ x to x (assumptions + expressions)
  ...
  ⊔syn-valid {υ₁ = υ₁} {υ₂ = υ₂} s₁ s₂ m₁ m₂
    with HasDecEq._≟_ exp-decEq (s₁ .σ .↓) Exp.□
       | HasDecEq._≟_ exp-decEq (s₂ .σ .↓) Exp.□
  -- Both □: both valid are ↦□, join is ↦□
  ... | yes refl | yes refl with s₁ .valid | s₂ .valid
  ...   | ↦□ | ↦□ = ↦□
  -- Left □: identity + minimality
  ⊔syn-valid {υ₁ = υ₁} {υ₂ = υ₂} s₁ s₂ m₁ m₂
    | yes σ₁≡ | no ne₂ with s₁ .valid
  ...   | ↦□ rewrite ⊔t-zeroₗ {υ₂ .↓} =
    ⊔-reduce-left (s₁ .γ) (s₂ .γ) (s₁ .σ) (s₂ .σ) (s₂ .valid)
      (min-γ≡⊥ s₁ m₁ refl) σ₁≡
  -- Right □: symmetric
  ⊔syn-valid {υ₁ = υ₁} {υ₂ = υ₂} s₁ s₂ m₁ m₂
    | no ne₁ | yes σ₂≡ with s₂ .valid
  ...   | ↦□ rewrite ⊔t-zeroᵣ {υ₁ .↓} =
    ⊔-reduce-right (s₁ .γ) (s₂ .γ) (s₁ .σ) (s₂ .σ) (s₁ .valid)
      (min-γ≡⊥ s₂ m₂ refl) σ₂≡
  -- Both ne: eliminate ⊑□ once, then split on D
  ⊔syn-valid s₁ s₂ m₁ m₂
    | no ne₁ | no ne₂ with s₁ .σ .proof | s₂ .σ .proof
  ... | ⊑□ | _ = ⊥-elim (ne₁ refl)
  ... | _ | ⊑□ = ⊥-elim (ne₂ refl)
  ⊔syn-valid {D = ↦*} s₁ s₂ m₁ m₂
    | no _ | no _ | ⊑* | ⊑* with s₁ .valid | s₂ .valid
  ...   | ↦* | ↦* = ↦*
  ⊔syn-valid {D = ↦Var _} s₁ s₂ m₁ m₂
    | no _ | no _ | ⊑Var | ⊑Var with s₁ .valid | s₂ .valid
  ...   | ↦Var p₁ | ↦Var p₂ = ↦Var (at-⊔ {s₁ .γ .↓} {s₂ .γ .↓} p₁ p₂)
  -- ↦∘
  ⊔syn-valid {D = ↦∘ D-fn eq D-arg} s₁ s₂ m₁ m₂
    | no _ | no _ | ⊑∘ σ₁fn⊑ σ₁arg⊑ | ⊑∘ σ₂fn⊑ σ₂arg⊑ with s₁ .valid | s₂ .valid
  ... | ↦∘ {τ = α₁} v₁-fn eq₁ v₁-arg | ↦∘ {τ = α₂} v₂-fn eq₂ v₂-arg =
    ↦∘ (⊔syn-valid fn-s₁ fn-s₂ min-fn₁ min-fn₂)
    ...
    where α₁⊑τ = syn-precision (s₁ .γ .proof) σ₁fn⊑ D-fn v₁-fn
          ...
          ... | ⊑∘ fn⊑ _ , γ⊑ = fn⊑ , γ⊑
          postulate ne-fn₁ : fn-s₁ .σ .↓ ≢ Exp.□
                    ne-fn₂ : fn-s₂ .σ .↓ ≢ Exp.□
  -- ↦&
  ⊔syn-valid {D = ↦& D₁ D₂} s₁ s₂ m₁ m₂
    | no _ | no _ | ⊑& σ₁₁⊑ σ₁₂⊑ | ⊑& σ₂₁⊑ σ₂₂⊑ with s₁ .valid | s₂ .valid
  ... | ↦& v₁₁ v₁₂ | ↦& v₂₁ v₂₂ =
    ↦& (⊔syn-valid fst-s₁ fst-s₂ min-fst₁ min-fst₂)
       (⊔syn-valid snd-s₁ snd-s₂ min-snd₁ min-snd₂)
    where α₁⊑ = syn-precision (s₁ .γ .proof) σ₁₁⊑ D₁ v₁₁
          ...
          postulate min-fst₁ : IsMinimal fst-s₁
                    ...
                    min-snd₂ : IsMinimal snd-s₂
  -- ↦π₁
  ⊔syn-valid {D = ↦π₁ D' eq} s₁ s₂ m₁ m₂
    | no _ | no _ | ⊑π₁ σ₁⊑ | ⊑π₁ σ₂⊑ with s₁ .valid | s₂ .valid
  ... | ↦π₁ {τ = α₁} v₁ eq₁ | ↦π₁ {τ = α₂} v₂ eq₂ =
    ↦π₁ (⊔syn-valid sub-s₁ sub-s₂ min-sub₁ min-sub₂)
  ...
  ... | ↦π₂ {τ = α₁} v₁ eq₁ | ↦π₂ {τ = α₂} v₂ eq₂ =
    ...
    where α₁⊑τ = syn-precision (s₁ .γ .proof) σ₁⊑ D' v₁
          ...
          postulate ⊔-product-distrib : ∀ {α₁ α₂ d₁ c₁ d₂ c₂}
                      ...
                      → (α₁ ⊔ α₂) ⊔ Typ.□ × Typ.□ ≡ (d₁ ⊔ d₂) × (c₁ ⊔ c₂)
  -- ↦λ:
  ⊔syn-valid {D = ↦λ: wf D'} s₁ s₂ m₁ m₂
    | no _ | no _ | ⊑λ τ₁⊑ σ₁⊑ | ⊑λ τ₂⊑ σ₂⊑ with s₁ .valid | s₂ .valid
  ... | ↦λ: wf₁ v₁ | ↦λ: wf₂ v₂ =
    ↦λ: wf-join (⊔syn-valid sub-s₁ sub-s₂ min-sub₁ min-sub₂)
    where α₁⊑ = syn-precision (⊑∷ τ₁⊑ (s₁ .γ .proof)) σ₁⊑ D' v₁
          ...
          sub-s₂ : SynSlice D' (↑ α₂⊑)
          sub-s₂ = record { γ = ↑ (⊑∷ τ₂⊑ (s₂ .γ .proof)) ; σ = ↑ σ₂⊑ ; valid = v₂ }
          -- BLOCKED: annotation type in sub-slice may differ from τ₁₁
          postulate min-sub₁ : IsMinimal sub-s₁
                    min-sub₂ : IsMinimal sub-s₂
                    wf-join : _ ⊢wf _
  -- ↦def
  ⊔syn-valid {D = ↦def D₁ D₂} s₁ s₂ m₁ m₂
    | no _ | no _ | ⊑def σ₁₁⊑ σ₁₂⊑ | ⊑def σ₂₁⊑ σ₂₂⊑ with s₁ .valid | s₂ .valid
  ... | ↦def v₁₁ v₁₂ | ↦def v₂₁ v₂₂ =
    ↦def (⊔syn-valid def-s₁ def-s₂ min-def₁ min-def₂)
         (⊔syn-valid body-s₁ body-s₂ min-body₁ min-body₂)
    where α₁⊑ = syn-precision (s₁ .γ .proof) σ₁₁⊑ D₁ v₁₁
          α₂⊑ = syn-precision (s₂ .γ .proof) σ₂₁⊑ D₁ v₂₁
          def-s₁ : SynSlice D₁ (↑ α₁⊑)
          ...
          postulate min-def₁  : IsMinimal def-s₁
                    ...
                    min-body₂ : IsMinimal body-s₂
  -- ↦Λ, ↦<>, ↦case: postulated (context shifting / computed result types)
  ⊔syn-valid {D = ↦Λ _} {υ₁} {υ₂} s₁ s₂ _ _
    | no _ | no _ | _ | _ = ⊔-case-Λ
    where postulate ⊔-case-Λ : _ ； (s₁ .γ ⊔ₛ s₂ .γ) .↓ ⊢ (s₁ .σ ⊔ₛ s₂ .σ) .↓ ↦ (υ₁ .↓ ⊔ υ₂ .↓)
  ⊔syn-valid {D = ↦<> _ _ _} {υ₁} {υ₂} s₁ s₂ _ _
    | no _ | no _ | _ | _ = ⊔-case-<>
    where postulate ⊔-case-<> : _ ； (s₁ .γ ⊔ₛ s₂ .γ) .↓ ⊢ (s₁ .σ ⊔ₛ s₂ .σ) .↓ ↦ (υ₁ .↓ ⊔ υ₂ .↓)
  ⊔syn-valid {D = ↦case _ _ _ _ _} {υ₁} {υ₂} s₁ s₂ _ _
    | no _ | no _ | _ | _ = ⊔-case-case
