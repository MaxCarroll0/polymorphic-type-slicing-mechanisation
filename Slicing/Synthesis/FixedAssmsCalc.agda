...
open import Slicing.MinSub using (tyarg; tyapp-decomp; tyarg-min; match-α-∀-mono)
open import Slicing.Synthesis.FixedAssmsSynthesis
open import Slicing.Synthesis.BranchPair

-- Inductive calculus generating term-minimal slices.
-- Dissertation: §8.5 Calculating Term-Minimal Slices.
...
data _◂_⤳_⇑_⊣_ {n : ℕ} {Γ : Assms} : ∀ {e : Exp} {τ : Typ}
  ...
  -- §7.5.3 Type Application: matches the type function's type against υ to
  -- collect α-constraints (their join is the sliced type argument `tyarg υ`)
  -- and a matched query `match-α υ` keeping α at α-positions.  The recursive
  -- premise slices the type function under the matched query, wrapped under
  -- ∀ for the outer-type's join shape.
  min<>    : ∀ {e τ τ' σ ψ-body γ σ-e}
               {D : n , Γ ⊢ e ⇑ τ} {m : τ ⊔ ∀· □ ≡ ∀· τ'} {wf : n ⊢wf σ}
               {υ : ⌊ [ zero ↦ σ ] τ' ⌋}
             → (υ .↓ ≢ □)
             → D ◂ (unmatch∀ m (match-α {τ'} {σ} υ)) ⤳ σ-e ⇑ ψ-body ⊣ γ
             → (⇑<> D m wf) ◂ υ ⤳ <>ₛ σ-e (tyarg {τ'} υ)
               ⇑ subₛ (tyarg {τ'} υ) (body∀ₛ ψ-body m) ⊣ γ
  ...
  mincase-desc
           : ∀ {e e₁ e₂ τ τ₁ τ₂ τ₁' τ₂' ς₁ ς₂ ψ₀ ψ₁ ψ₂ υ₁ υ₂ γ₀ γ₁ γ₂ σ₀ σ₁ σ₂}
             ...
             → IsCaseBranchPairMin D m D₁ D₂ σ₀ σ₁ σ₂ υ ψ₀ ς₁ ς₂
...
-- e ⟨σ⟩ : υ - §7.5.3 type application, factored through match-tyapp.
extract' (min<> {τ = τ} {τ' = τ'} {σ = σ} {D = D} {m = m} {wf = wf} {υ = υ} υ≢□ sub)
  with extract' sub
... | ((σ-e ⇑ ψ-body ∈ d ⊒ v) , ih-min) , ≡refl , ≡refl
  with ⊔-∀-⊑ v (match∀ₛ ψ-body m)
... | _ , m'' , υ'⊑body
  rewrite ≡sym (unmatch∀-≡ {τ} m _ m'')
  = (s , min) , ≡refl , ≡refl
  where
    s = <>ₛ σ-e (tyarg {τ'} {σ} υ) ⇑ subₛ (tyarg {τ'} {σ} υ) (body∀ₛ ψ-body m)
        ∈ ⇑<> d (match∀ₛ ψ-body m) (wf-⊑ wf (tyarg {τ'} {σ} υ .proof))
        ⊒ ⊑.trans {Typ} (tyapp-decomp {τ'} {σ} υ)
                        (sub-⊑ zero (⊑.refl {Typ}) υ'⊑body)
    ∀·-inj : ∀ {a b : Typ} → ∀· a ≡ ∀· b → a ≡ b
    ∀·-inj ≡refl = ≡refl
    ...
    ... | τ₃⊑τ | _ , m₃ , τ₃body⊑
      with ih-min (↑ p₁ ⇑ ↑ τ₃⊑τ ∈ d'
                     ⊒ match-α-∀-mono m υ υ≢□ τ₃⊑τ m' p₂ v') e⊑
    ... | ≡refl
      rewrite ∀·-inj (≡trans (≡sym m₃) m')
      with ⊑.antisym {Typ} (tyarg-min {τ'} {σ} υ (↑ p₂) (↑ τ₃body⊑) v') τ⊑
    ... | ≡refl = ≡refl
...
... | ((σ₁ ⇑ ψ₁e ∈ d₁ ⊒ v₁) , ih₁) , ≡refl , ≡refl
  ...
  where
    ...
    ... | _ , d₁-fst , p₁-fst | _ , d₂-snd , p₂-snd
      ...
      with ς₁⊑τ₃ , ς₂⊑τ₄ ← head-min-witness head-min e₀⊑ d₀' m' d₁-fst d₂-snd υ-cov-lifted
      with ih₀ (↑ p₀ ⇑ ↑ τ₀⊑ ∈ d₀'
                  ⊒ unmatch+-min-⊑ τ m ς₁ ς₂ τ₀⊑ m' ς₁⊑τ₃ ς₂⊑τ₄) e₀⊑
    ... | ≡refl = ≡refl
...
-- e ⟨σ⟩ : υ - §7.5.3 type application, substituting tyarg into body
extract-ctx (min<> {τ = τ} {τ' = τ'} {σ = σ} {m = m} {wf = wf} {υ = υ} υ≢□ sub)
  with extract-ctx sub
... | ψ' , d' , v'
  with ⊔-∀-⊑ v' (match∀ₛ ψ' m)
... | _ , m' , v''
  rewrite ≡sym (unmatch∀-≡ {τ} m _ m')
  = subₛ (tyarg {τ'} {σ} υ) (body∀ₛ ψ' m)
  , ⇑<> d' (match∀ₛ ψ' m) (wf-⊑ wf (tyarg {τ'} {σ} υ .proof))
  , ⊑.trans {Typ} (tyapp-decomp {τ'} {σ} υ)
                  (sub-⊑ zero (⊑.refl {Typ}) v'')
...
... | τ⊑
  = extract-ctx-min sub d'
      (match-α-∀-mono m υ υ≢□ τ⊑ m' (tyarg {τ'} {σ} υ .proof) v)
      Γ'⊑
