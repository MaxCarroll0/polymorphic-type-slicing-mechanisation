...
-- Bidirectional typing relations Γ ⊢ e ⇑ τ (synthesis) and Γ ⊢ e ⇓ τ (analysis) for the core calculus.
-- Dissertation: §4.3 Typing Rules (figs:synthesis-rules, analysis-rules).
...
mutual
  data _,_⊢_⇑_ : ℕ → Assms → Exp → Typ → Set where
    ...
    -- Dynamic Sum injections synthesising: ι₁ e ⇑ τ + □ and ι₂ e ⇑ □ + τ where e ⇑ τ.
    ⇑ι₁  : ∀ {n : ℕ} {Γ : Assms} {e : Exp} {τ : Typ} →
              n , Γ ⊢ e ⇑ τ              →
              --------------------
              n , Γ ⊢ ι₁ e ⇑ τ + □
