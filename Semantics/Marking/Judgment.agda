-- Error-marking judgment Γ ⊢ e ↬ ě ⇑/⇓ τ that maps unmarked expressions to marked ones.
-- Dissertation: §7.1 Error Marking (sec:marking-calculus, marking.tex).
...
mutual
  ...
  data _,_⊢_↬_⇑_ : ℕ → Assms → Exp → MExp → Typ → Set where
    ...
    -- Sum injections synthesise τ + □ / □ + τ where focus synthesises τ
    mark⇑ι₁    : ∀ {n Γ e ě τ} →
                   n , Γ ⊢ e ↬ ě ⇑ τ →
                   n , Γ ⊢ ι₁ e ↬ ι₁ ě ⇑ τ + □
